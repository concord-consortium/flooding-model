...
const setupAlpha = (geometry: THREE.PlaneGeometry, simulation: SimulationModel) => {
...
export const Water = observer(function WrappedComponent() {
  ...
  useLayoutEffect(() => {
    geometryRef.current?.setAttribute("alpha",
  ...
  useLayoutEffect(() => {
    if (simulation.config.useGPU || !geometryRef.current) {
      return;
    }
