...
const getNewFlux = (dt: number, oldFlux: number, heightDiff: number, cellArea: number) => {
  return Math.max(0, oldFlux + GRAVITY * PIPE_FACTOR * cellArea * dt * heightDiff) * DAMPING_FACTOR;
};
...
export class FloodingEngine {
  ...
  public updateFlux(dt: number) {
    ...
    for (const cell of this.activeCells) {
      ...
      nFluxL = !nCell.isEdge ? getNewFlux(dt, cell.fluxL, cell.elevation - nCell.elevation, cellArea) : 0;
      // fluxR
      nCell = this.getCellAt(x + 1, y);
      nFluxR = !nCell.isEdge ? getNewFlux(dt, cell.fluxR, cell.elevation - nCell.elevation, cellArea) : 0;
      // fluxB
      nCell = this.getCellAt(x, y - 1);
      nFluxB = !nCell.isEdge ? getNewFlux(dt, cell.fluxB, cell.elevation - nCell.elevation, cellArea) : 0;
      // fluxT
      nCell = this.getCellAt(x, y + 1);
      nFluxT = !nCell.isEdge ? getNewFlux(dt, cell.fluxT, cell.elevation - nCell.elevation, cellArea) : 0;
